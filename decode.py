...
from lhotse import load_manifest_lazy, set_caching_enabled
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="""Optional path to a single .pt checkpoint to decode directly.
        When set, --epoch/--avg/--use-averaged-model are ignored and this file
        is loaded as-is. Use it to decode an already-averaged base checkpoint
        (e.g. epoch-35-avg-6.pt) or any single fine-tuned epoch-N.pt.""",
    )

    parser.add_argument(
        "--test-manifest",
        type=str,
        default="vibank_cs_cuts_test.jsonl.gz",
        help="""Filename (under --manifest-dir) of the test cuts to decode,
        e.g. produced by local/compute_fbank_huggingface.py.""",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="vibank_cs",
        help="Display name for the test set in result and WER filenames.",
    )
...
def main():
    ...
    if params.checkpoint is not None:
        logging.info(f"Loading checkpoint directly from {params.checkpoint}")
        model.to(device)
        load_checkpoint(params.checkpoint, model)
    ...
    # Reused only for its dataloader plumbing; cuts come from --test-manifest.
    librispeech = LibriSpeechAsrDataModule(args)

    test_cuts = load_manifest_lazy(Path(args.manifest_dir) / params.test_manifest)
    test_dl = librispeech.test_dataloaders(test_cuts)

    test_sets = [params.test_set_name]
    test_dls = [test_dl]
