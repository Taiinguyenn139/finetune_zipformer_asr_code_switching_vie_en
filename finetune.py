...
# Set to True by setup_mlflow() on rank 0 once an MLflow run is active.
_USE_MLFLOW = False


def maybe_log_mlflow_metrics(metrics: Dict[str, float], step: int) -> None:
    """Log scalar metrics to MLflow if enabled.

    This is a no-op when MLflow is disabled and never raises, so a logging or
    network failure (e.g. an unreachable Databricks workspace) cannot kill a
    long training run.
    """
    if not _USE_MLFLOW:
        return
...
def setup_mlflow(params: AttributeDict) -> None:
    ...
    Auth is read from the environment (MLFLOW_TRACKING_URI=databricks plus
    DATABRICKS_HOST / DATABRICKS_TOKEN). The run id is persisted under exp_dir
    so resuming training continues the same run instead of fragmenting curves.
    """
    global _USE_MLFLOW
    try:
        import mlflow
    except ImportError:
        logging.warning("mlflow is not installed; disabling MLflow logging.")
        return

    if params.mlflow_experiment:
        mlflow.set_experiment(params.mlflow_experiment)

    run_id_file = params.exp_dir / "mlflow_run_id.txt"
    run_id = None
    ...
    try:
        if run_id:
            ...
            run = mlflow.start_run(run_name=params.mlflow_run_name)
            run_id_file.write_text(run.info.run_id)
            logging.info(f"Started MLflow run {run.info.run_id}")
    except Exception as e:  # noqa
        logging.warning(f"Could not start MLflow run: {e}; disabling MLflow.")
        return

    # Log a flat, stringified subset of hyperparameters (MLflow params must be
    # scalar/str; `params` contains nested objects such as env_info).
    hparams = {
        ...
        "use_mux": params.use_mux,
        "bpe_model": params.bpe_model,
        "train_manifest": params.train_manifest,
        "valid_manifest": params.valid_manifest,
        "context_size": params.context_size,
        "prune_range": params.prune_range,
        "vocab_size": params.vocab_size,
        "finetune_ckpt": params.finetune_ckpt,
    }
    try:
        mlflow.log_params({k: str(v) for k, v in hparams.items()})
    except Exception as e:  # noqa
        logging.warning(f"MLflow param logging failed: {e}")
...
def add_finetune_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--train-manifest",
        type=str,
        default="mydata_cuts_train.jsonl.gz",
        help="""Filename (under --manifest-dir) of the training cuts to
        fine-tune on, e.g. produced by local/compute_fbank_huggingface.py.""",
    )

    parser.add_argument(
        "--valid-manifest",
        type=str,
        default="mydata_cuts_dev.jsonl.gz",
    ...
    parser.add_argument(
        ...
        type=str,
        default="mydata",
        help="Display name for the validation set in logs and metrics.",
    )
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="""If true, log metrics and params to MLflow (rank 0 only).
        Authentication is taken from the environment, e.g.
        MLFLOW_TRACKING_URI=databricks plus DATABRICKS_HOST/DATABRICKS_TOKEN.""",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="""MLflow experiment path, e.g. /Users/<email>/zipformer-finetune.
        Required by Databricks-hosted MLflow.""",
    )
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if batch_idx % params.log_interval == 0:
            ...
            maybe_log_mlflow_metrics(
                {
                    "train/learning_rate": cur_lr,
                    "train/loss": loss_info["loss"] / max(loss_info["frames"], 1),
                    "train/tot_loss": tot_loss["loss"] / max(tot_loss["frames"], 1),
                    **(
        ...
        if batch_idx % params.valid_interval == 0 and not params.print_diagnostics:
            for valid_set, valid_dl in zip(valid_sets, valid_dls):
                ...
                maybe_log_mlflow_metrics(
                    {
                        f"valid/{valid_set}_loss": valid_info["loss"]
...
def run(rank, world_size, args):
    ...
    if rank == 0 and params.use_mlflow:
        setup_mlflow(params)
    ...
    # Fine-tune on a user-provided dataset (e.g. prepared from a HuggingFace
    # dataset via local/compute_fbank_huggingface.py). Manifests are read
    # directly from --manifest-dir, so no corpus-specific data-module method is
    # required.
    train_cuts = load_manifest_lazy(args.manifest_dir / params.train_manifest)
    if params.use_mux:
        raise NotImplementedError(
            "use_mux is not configured for custom fine-tuning data. To mix in "
            "original-domain data, load those cuts here and set the mux weights "
            "explicitly, or run with --use-mux 0 to fine-tune on your data only."
        )
    ...
    def remove_short_and_long_utt(c: Cut):
        ...
        # the threshold
        if c.duration < 2.0 or c.duration > 20.0:
            # logging.warning(
    ...
    valid_cuts = load_manifest_lazy(args.manifest_dir / params.valid_manifest)

    valid_sets = [params.valid_set_name]
    valid_dls = [
